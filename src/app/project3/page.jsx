...
export default function Project1() {
  ...
  const [hackedTitle, setHackedTitle] = useState("PersonalProject");
  ...
  const startTitleHack = () => {
    ...
    const hackInterval = setInterval(() => {
      iterations++;
      const randomText = Array("PersonalProject".length)
        .fill()
      ...
      if (iterations >= maxIterations) {
        ...
        setHackedTitle("PersonalProject");
      }
    }, 50);
  };

  const startFooterHack = () => {
    if (!isMobile) {  // Alleen uitvoeren op desktop
      const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#$%^&*';
      let iterations = 0;
      const maxIterations = 3;
      const originalText = "LET'S CHAT";
      
      const hackInterval = setInterval(() => {
        iterations++;
        const randomText = Array(originalText.length)
          .fill()
          .map((_, i) => originalText[i] === "'" || originalText[i] === " " 
            ? originalText[i] 
            : characters[Math.floor(Math.random() * characters.length)])
          .join('');
        
        const footerTitle = document.querySelector('.footer-title');
        if (footerTitle) footerTitle.textContent = randomText;
        
        if (iterations >= maxIterations) {
          clearInterval(hackInterval);
          if (footerTitle) footerTitle.textContent = originalText;
        }
      }, 50);
    }
  };
